...
# Clean up any leftover debris line
text = text.replace('ارح البحر بن عبد الله الحضرمي\n٠ط\n', '')
text = text.replace('٠ط\n', '')

with open('source_lineage.txt', 'w', encoding='utf-8') as f:
    f.write(text)

# Also update rawSharhAlBahrText.ts
    ...
    def finalize_block():
        ...
        if not matches:
            ...
            for i, m in enumerate(matches):
                ...
                # Special handling for distinct names
                if 'اسكندر' in raw_val:
                    name_clean = "الشيخ لطف الله ( الملقب اسكندر ) بن عبد الرحمن ساكن النقل بن أحمد بن عبد الله الحضرمي"
                ...
                elif 'الشيخ / عبد الله' in raw_val or 'الشيخ عبد الله' in raw_val:
                    if '29' in code_str:
                        name_clean = "الشيخ عبد الله بن عبد الرحمن النقل بن شارح البحر أحمد بن عبد الله الحضرمي"
    ...
    while i < len(lines):
        ...
        if re.match(r'^27\s*[/／]\s*1\s*-\s*شارح البحر', line):
            ...
            if i < len(lines) and re.match(r'^(?:و?\s*أعقب|و?\s*اعقب)', lines[i].strip()):
                i += 1
            # Add lines up to 3/28
            while i < len(lines) and not re.search(r'(?:3/28|28/3)\s*[-–—=/_*#@:]+\s*(?:اعقاب|أعقاب)', lines[i].strip()):
                if lines[i].strip():
        ...
        # Abd Al-Rahman Sakin Al-Naql 3/28
        if re.search(r'(?:3/28|28/3)\s*[-–—=/_*#@:]+\s*(?:اعقاب|أعقاب)\s*الشيخ\s*/?\s*عبد الرحمن ساكن النقل', line):
            finalize_block()
            current_father_gen = 28
            current_father_code = "28/3"
            current_father_name = "الشيخ عبد الرحمن ساكن النقل بن شارح البحر أحمد بن عبد الله الحضرمي"
            i += 1
            if i < len(lines) and re.match(r'^(?:و?\s*أعقب|و?\s*اعقب)', lines[i].strip()):
                i += 1
            # Add 29/1, 29/2, 29/3, 29/4 as children
            current_children_raw.append("29/ 1 - الشيخ / لطف الله ( الملقب اسكندر ) بن عبد الرحمن ساكن النقل بن أحمد بن عبد الله الحضرمي [ ولا نعلم له عقب ]")
            current_children_raw.append("29/ 2 - الشيخ / عبد الله بن عبد الرحمن النقل بن شارح البحر أحمد بن عبد الله الحضرمي [ولا نعلم له عقب]")
            current_children_raw.append("29/ 3 - الشيخ / أحمد بن عبد الرحمن النقل بن شارح البحر أحمد الحضرمي (له عقب)")
            current_children_raw.append("29/ 4 - الشيخ محمد جمال الدين بن عبد الرحمن النقل بن أحمد شارح البحر بن عبد الله الحضرمي (له عقب)")
            finalize_block()
            
            # Fast forward to 29/3 father header
            while i < len(lines) and not re.match(r'^29\s*[/／]\s*3\s*-\s*الشيخ\s*/?\s*أحمد بن عبد الرحمن النقل', lines[i].strip()):
                i += 1
            continue
